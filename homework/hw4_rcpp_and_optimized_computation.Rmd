...
The last inner prod result disagrees with the previous ones every time I tried to run the code. Sometimes I got Inf for subseq_val. The reason is that vector in C starts from 0 instead of 1. This vector in C should be from 0 to 1022. The 1023 th element of that vector in C will be out of memory and get some unreasonable result.
...
The total dimension is 4*2. Element in each column is the same. The first column is always 1024 and the second column is not the same each time I re-run the code. Matrix in R is stored in column-major. Since the number of columns of A is 1024, C++ will try to get first 1024 element in memory to do the matrix multiplication. That's why the first column is always 1024. For the second column, C++ will get a number out of the memory of B. That's why the second column is not the same each time.
...
R session aborted when I run the first code chunk since the size of y is 1000 and less than $10^7$. We are trying to get the element out of the memory of y.

For the second code chunk, the condition for executing the for loop is i<1000. We only trying to get the first 1000 elements of x and y.  
...
all.equal(row_col_dot_matmat(A, B), A %*% B)
```
...
Time of `col_oriented_matmat` is 813 ms (median). Time of `row_col_dot_matmat` is 6.01 s (median). `col_oriented_matmat` is faster. R stored matrix in column-major and looping through rows will take more time to get the element in memory.
...
`%*%` takes about 490 ms (median) and is the fastest.
```{r}
bench::mark(A %*% B)
```
...
The test runs on JHPCE compute node 127. For default, the median time of dgemm_eigen is 180 ms. Our method takes about 829 ms. By turning on AVX, the median time is 92.6ms. By turning on FMA, the median time of dgemm_eigen is 65.2ms.  Our method takes about 830ms. By turning on AVX and FMA, the median time of dgemee_eigen is 64.5ms. Our method take about 960ms. By turning on AVX, FMA, and tuning off SSE, the median time of dgemm_eigen is 86.5ms. Our method takes about 955ms. By turning on all method, the median time of dgemm_eigen is 87.9ms. Our method takes about 954ms. 

Our method did not have improvements using different compiler. The speed of dgemm_eigen increases a lot.
