...
The size of the folder is much larger then the sizes of the individual files inside it. Since each time we used the "git commit", the files in objects will provide a snapshot of the repo at the time of the commit. Git knows not to make a new file for unchanged files. It will make a new file for the changed files (foo.tex and foo.pdf).

If we want to commit large binary files, it will create a new file for the changed files every time. The speed is slow. It will also increase the storage.
...
https://github.com/YuzhengDun1999/hiperglm