...
The estimate does not make sense. Denote $x$ as the true sample mean and $y$ as the true sample second moment, $x(1+\delta_x)$ as the approximation for the sample mean in the algorithm, $\delta_x$ is the relative error and $y(1+\delta_y)$ as the approximation for the sample second moment in the algorithm, $\delta_y$ is the relative error. We have that:
$$x(1+\delta_x)-y(1+\delta_y)=(x-y)(1+\frac{x\delta_x-y\delta_y}{x-y})$$
The relative error is $|\frac{x\delta_x-y\delta_y}{x-y}|$. The theoretical true value of $x-y$ is 1. However, the $x$ and $y$ are very large in this example. In this case, relative error will be very large.

The estimate changed a lot if we use different seed.

the distribution of the estimator `sample_var_with_question_mark` under repeated Monte Carlo simulations is normal distribution.
...
I expect there will be a straight line with negative slope. The relative error plot does not agree with what the mathematical theory predicts. I observe the relative errors first decrease and then increase as the stepsize becomes smaller.

Also, what is the slope in the "latter part" of your plotted line, roughly speaking? Explain why we observe such a slope in the relative error. 
...
The slope is roughly equal to 1 in the "latter part" of the plotted line. From the Taylor expansion in the lecture, the difference between the numerical differentiation and true analytical differentiation is $O((\Delta x)^2)$. When the stepsize is so small, the machine cannot calculate the true difference between $f(x+\Delta x)-f(x-\Delta x)$. The error is given by the machine error divided by $\Delta x$. If stepsize $\Delta x$ is too small, $O((\Delta x)^2)$ can be ignored and the error is mainly contributed by machine error divided by $\Delta x$. In the log-log scale, the slope will be roughly equal to 1.
...
https://github.com/YuzhengDun1999/hiperglm/pull/1